...
//取得套接字句柄
    ...
    bool Socket::InitLocalAddress() {
        ...
        memset(&sockAddr, 0, sizeof(sockAddr));
        socklen_t nSockAddrLen = sizeof(sockAddr);
        if (SOCKET_ERROR == getsockname(m_hSocket, (sockaddr*) &sockAddr, &nSockAddrLen))
            return false;
 ...
 */
    bool Socket::Bind(unsigned short nPort, char* strIP) {
        ...
        return (SOCKET_ERROR != bind(m_hSocket, (sockaddr*) pAddr, sizeof(sockaddr_in)));
    }
 ...
 */
    int Socket::SendTo(const char* strIP, int nPort, const void* lpBuf, int nBufLen, int nFlags) {
        ...
        int ret = (int) sendto(m_hSocket, (const char*) lpBuf, nBufLen, nFlags, (sockaddr*) &sockAddr, sizeof(sockaddr));
        if (ret < 0)
            ret = -1;
        return ret;
    }
 ...
 */
    int Socket::ReceiveFrom(char* lpBuf, int nBufLen, std::string& strFromIP, int& nFromPort, bool bCheckDataLength, long lSecond, long lMinSecond) {
        ...
        while (true) {
            ...
            if (bCheckDataLength)
                nFlag = MSG_PEEK;
            nResult = (int) recvfrom(m_hSocket, lpBuf, nBufLen, nFlag, (sockaddr*) &sockAddr, &nAddrLen);
            if (nAddrLen > 0)
                GetAddress(&sockAddr, strFromIP, nFromPort);
    ...
    bool Socket::SetNoDelay(bool yes) {
        int opt = yes ? 1 : 0;
        return -1 != setsockopt(m_hSocket, IPPROTO_TCP, TCP_NODELAY, (char*) &opt, sizeof(opt));

    }

    bool Socket::SetNoDelay(int sock, bool yes) {
        int opt = yes ? 1 : 0;
        return -1 != setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char*) &opt, sizeof(opt));

    }

    bool Socket::SetSendBufSize(int buffsize) {
        return -1 != setsockopt(m_hSocket, SOL_SOCKET, SO_SNDBUF, (char*) &buffsize, sizeof(buffsize));

    }

    bool Socket::SetSendBufSize(int sock, int buffsize) {
        return -1 != setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char*) &buffsize, sizeof(buffsize));

    }

    bool Socket::SetRecvBufSize(int buffsize) {
        return -1 != setsockopt(m_hSocket, SOL_SOCKET, SO_RCVBUF, (char*) &buffsize, sizeof(buffsize));

    }

    bool Socket::SetRecvBufSize(int sock, int buffsize) {
        return -1 != setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char*) &buffsize, sizeof(buffsize));
    ...
    bool Socket::SetSendTimeout(long sec, long usec) {
        ...
        return -1 != setsockopt(m_hSocket, SOL_SOCKET, SO_SNDTIMEO, (char*) &timeo, len);
    ...
    bool Socket::SetSendTimeout(int sock, long sec, long usec) {
        struct timeval timeo = {sec, usec};
        socklen_t len = sizeof(timeo);
        return -1 != setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (char*) &timeo, len);
    ...
    bool Socket::SetRecvTimeout(long sec, long usec) {
        struct timeval timeo = {sec, usec};
        socklen_t len = sizeof(timeo);
        return -1 != setsockopt(m_hSocket, SOL_SOCKET, SO_RCVTIMEO, (char*) &timeo, len);
    ...
    bool Socket::SetRecvTimeout(int sock, long sec, long usec) {
        struct timeval timeo = {sec, usec};
        socklen_t len = sizeof(timeo);
        return -1 != setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char*) &timeo, len);
