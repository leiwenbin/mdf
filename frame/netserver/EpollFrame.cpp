...
namespace mdf {
    ...
    void EpollFrame::DataMonitor() {
        ...
        while (!m_stop) {
            ...
            nCount = MAXPOLLSIZE;
            if (0 >= ioList.size())
                ret = ((EpollMonitor*) m_pNetMonitor)->WaitData(events, nCount, -1);
            else
                ret = ((EpollMonitor*) m_pNetMonitor)->WaitData(events, nCount, 0);
            if (!ret) break;
    ...
    void EpollFrame::SendAbleMonitor() {
        ...
        while (!m_stop) {
            ...
            nCount = MAXPOLLSIZE;
            if (0 >= ioList.size())
                ret = ((EpollMonitor*) m_pNetMonitor)->WaitSendable(events, nCount, -1);
            else
                ret = ((EpollMonitor*) m_pNetMonitor)->WaitSendable(events, nCount, 0);
            if (!ret) break;
