...
namespace mdf {
    ...
    bool NetEngine::Start() {
        ...
        int i = 0;
        for (i = 0; i < m_ioThreadCount; i++)
            m_ioThreads.Accept(Executor::Bind(&NetEngine::NetMonitorTask), this, NULL);
#ifndef WIN32
        for (i = 0; i < m_ioThreadCount; i++)
            m_ioThreads.Accept(Executor::Bind(&NetEngine::NetMonitorTask), this, (void*) 1);
        for (i = 0; i < m_ioThreadCount; i++)
            m_ioThreads.Accept(Executor::Bind(&NetEngine::NetMonitorTask), this, (void*) 2);
    ...
    void NetEngine::Stop() {
        if (m_stop) return;
        m_stop = true;
        if (NULL != m_pNetMonitor) m_pNetMonitor->Stop();
        m_sigStop.Notify();
    ...
    void NetEngine::CloseConnect(NetConnect* pNetConnect) {
         ...
         */
        NetConnect* pConnect = pNetConnect;
        ...
        if (itNetConnect != m_connectList.end()) m_connectList.erase(itNetConnect);
        /*
    ...
    bool NetEngine::Listen(int port) {
        ...
        pair<map<int, int>::iterator, bool> ret = m_serverPorts.insert(map<int, int>::value_type(port, INVALID_SOCKET));
...
// 	bool successed = sock.Connect(ip, port);
    ...
    void* NetEngine::ConnectThread(void*) {
        SVR_CONNECT** clientList = (SVR_CONNECT**) new mdf::uint64[20000];
        int clientCount = 0;
        unsigned int i = 0;
        SVR_CONNECT* pSvr = NULL;
#ifndef WIN32
        m_hEPoll = epoll_create(20000);
        mdf_assert(-1 != m_hEPoll);
        m_events = new epoll_event[20000]; //epoll事件
        mdf_assert(NULL != m_events);
#endif
            ...
            isEnd = true;
            {
                ...
                for (; it != m_keepIPList.end() && clientCount < 20000; it++) {
                    for (i = 0; i < it->second.size() && clientCount < 20000; i++) {
                        ...
                        mdf_assert(INVALID_SOCKET != pSvr->sock);
...
#include <netdb.h>
    ...
    NetEngine::ConnectResult NetEngine::AsycConnect(int svrSock, const char* lpszHostAddress, unsigned short nHostPort) {
        ...
        if ((hostinfo = gethostbyname(lpszHostAddress)) != NULL) strcpy(ip, (const char*) (inet_ntoa(*((struct in_addr*) (*(hostinfo->h_addr_list))))));
        ...
        if (SOCKET_ERROR != connect(svrSock, (sockaddr*) &sockAddr, sizeof(sockAddr))) return NetEngine::success;
#ifndef WIN32
        if (EINPROGRESS == errno) return NetEngine::waitReulst;
#else
    ...
    bool NetEngine::EpollConnect(SVR_CONNECT** clientList, int clientCount) {
        ...
        for (i = 0; i < clientCount; i++) {
            ...
            ev.data.ptr = pSvr;
            if (0 > epoll_ctl(m_hEPoll, EPOLL_CTL_ADD, pSvr->sock, &ev))
                pSvr->inEpoll = false;
            else {
                pSvr->inEpoll = true;
                monitorCount++;
    ...
    bool NetEngine::ConnectIsFinished(SVR_CONNECT* pSvr, bool readable, bool sendable, int api, int errorCode) {
        ...
        if (sendable) {
            nSendSize0 = (int) send(svrSock, buf, 0, 0);
            sendError0 = 0;
            if (0 > nSendSize0) {
#ifdef WIN32
        ...
        if (readable) {
            nRecvSize0 = (int) recv(svrSock, buf, 0, MSG_PEEK);
            recvError0 = 0;
