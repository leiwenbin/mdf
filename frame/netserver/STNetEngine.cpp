 ...
 */
    ...
    bool STNetEngine::LinuxIO(int timeout) {
        ...
        if (0 >= m_ioList.size())
            nCount = m_pNetMonitor->WaitEvent(timeout);
        else
            nCount = m_pNetMonitor->WaitEvent(0);
        ...
        for (i = 0; i < nCount; i++) {
            ...
            //不是监听socket一定是io事件
            //加入到io列表，统一调度
            if (m_pNetMonitor->IsWriteAble(i))
                eventType = 1 | 2; //recv+send事件
            else
                eventType = 1; //recv事件
...
#endif
        m_clientList = (SVR_CONNECT**) new mdf::uint64[20000];
        while (!m_stop) {
            if (!mainFinished) {
