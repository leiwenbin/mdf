...
#ifndef WIN32
        if (m_listenSockets.end() != m_listenSockets.find(sock))
            return false;
        m_listenSockets.insert(map<int, int>::value_type(sock, sock));
        epoll_event ev;
        ev.events = EPOLLIN | EPOLLET;
        ev.data.fd = sock;
        if (0 > epoll_ctl(m_hEpoll, EPOLL_CTL_MOD, sock, &ev))
            return false;
#endif
        return true;
    }
...
//增加一个IO操作，可IO时WaitEvent会返回
...
#ifndef WIN32
        if (epoll_ctl(m_hEpoll, EPOLL_CTL_DEL, sock, NULL) < 0)
            return false;
        map<int, int>::iterator it = m_listenSockets.find(sock);
        if (it != m_listenSockets.end())
            m_listenSockets.erase(it);
#endif
        return true;
    }
    ...
    bool STEpoll::AddMonitor(int sock, char* pData, unsigned short dataSize) {
        ...
        ev.data.fd = sock;
        if (0 > epoll_ctl(m_hEpoll, EPOLL_CTL_ADD, sock, &ev))
            return false;
#endif
        return true;
    }
