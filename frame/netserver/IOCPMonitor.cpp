...
namespace mdf {
    ...
    bool IOCPMonitor::Start(int nMaxMonitor) {
        ...
        m_hCompletPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, 0, 0, NumberOfConcurrentThreads);
...
#ifdef WIN32
        //将监听套接字加入IOCP列队
        if (NULL == CreateIoCompletionPort((HANDLE)sock, m_hCompletPort, (DWORD)sock, 0))
            return false;
#endif
        return true;
    }
    ...
    bool IOCPMonitor::WaitEvent(void* eventArray, int& count, bool block) {
        ...
        int sock;
        if (!::GetQueuedCompletionStatus(m_hCompletPort, &dwIOSize, (LPDWORD)&sock, (OVERLAPPED**)&pOverlapped, INFINITE))
        {
